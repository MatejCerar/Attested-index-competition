...
// Writes frontend/data.js for the leaderboard. Testnet, no real funds.
...
async function main() {
    ...
    const vaultArt = JSON.parse(
        readFileSync(join(root, "demo", "abi", "StableIndexVault.json"), "utf8")
    );
    const usdcArt = JSON.parse(
        readFileSync(join(root, "demo", "abi", "MockUSDC.json"), "utf8")
    );
    ...
    writeFileSync(
        join(root, "frontend", "data.js"),
        "window.DEMO = " + JSON.stringify(data, null, 2) + ";\n"
    );
