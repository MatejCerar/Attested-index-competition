...
//   {intervalMs, driftBps, takeProfitPct, cooldownMs, combine, maxStepMoveBps, note}
// ---------------------------------------------------------------------------
...
// survives. takeProfitPct accepts a fraction (0.1) or a percent (10).
...
export function evalRebalanceSpec(spec, ctx) {
    const {now, lastRebalanceAt, currentWeightsBps, targetWeightsBps, nav, lastRebalanceNav} =
        ctx || {};
    if (
    ...
    return {fire: true, reason: hits.map((c) => c.reason).join(" AND ")};
}
