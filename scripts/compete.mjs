...
// Results stream to app/public/data/live.json, shown by the app's /live route.
//
...
function snapshot(finished) {
    ...
    // Feed the app (plain JSON at app/public/data/live.json).
    writeFileSync(join(__dirname, "..", "app", "public", "data", "live.json"), JSON.stringify(data, null, 2) + "\n");
    return data;
...
const top = final.indices.filter((i) => !i.notExecutable)[0];
console.log(top ? `\nWINNER: ${top.name} at ${pctS(top.ret)} over ${DURATION}s. wrote app/public/data/live.json`
                : "\nno executable indices on this source. wrote app/public/data/live.json");
